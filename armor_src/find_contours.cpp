...
Mat Light_bar::find_contours(Mat frame, Mat hsv, vector<Armor>& temp_armors,double timestamp)
{
    ...
    for (int i = 0; i < contours.size(); i++)
    {
        if (contourArea(contours[i]) > 30)
        {
            ...
            float judge = max(minRect.size.width , minRect.size.height)/min(minRect.size.width , minRect.size.height);
            if(judge<3.5&&judge>1.2)
            {
            //合格装甲板存入lightbar
            lightbar.push_back(minRect);
            for (int j = 0; j < 4; j++)
            {
            //line(frame, rect_points[j], rect_points[(j + 1) % 4], Scalar(255, 0, 255), 1);
            }
            }
        }
    }
    ...
    for(int i=0;i<lightbar.size();i++)
    {
        ...
        {
            if(abs(lightbar[i].angle-lightbar[j].angle)<5&&abs(lightbar[i].center.y-lightbar[j].center.y)<20&&abs(lightbar[i].center.x-lightbar[j].center.x)<100)
            {
                //计算装甲板的旋转矩形
