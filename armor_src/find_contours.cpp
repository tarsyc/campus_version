...
Mat Light_bar::find_contours(Mat frame, Mat hsv, vector<Armor>& temp_armors,double timestamp)
{
    ...
    for(int i=0;i<lightbar.size();i++)
    {
        ...
        {
            if(abs(lightbar[i].angle-lightbar[j].angle)<10&&abs(lightbar[i].center.y-lightbar[j].center.y)<20&&abs(lightbar[i].center.x-lightbar[j].center.x)<100){
                //计算装甲板的旋转矩形
